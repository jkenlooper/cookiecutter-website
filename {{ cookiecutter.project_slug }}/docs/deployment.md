...
Run the below commands to remove {{ cookiecutter.project_slug }} from the
server.  This will uninstall and disable the services, remove any files
installed outside of the `/usr/local/src/{{ cookiecutter.project_slug }}/`
directory including the sqlite3 database and finally remove the source files as
well.  *Only do this if the website is running on the new server and is no
longer needed on the old one.*
...
cd ../;
sudo rm -rf /usr/local/src/{{ cookiecutter.project_slug }}/;
```
